...
> eval3 :: (MonadFail m, Monad m)  => Exp1 -> m Int
...
> eval4 :: (MonadFail m, Monad m)  => Exp1 -> m Int
...
>   fmap f (RoseNode x xs) = RoseNode (f x) ((fmap f) <$> xs)


> instance Applicative (Rose) where
>   pure v = RoseNode v [RoseLeaf]
>   (RoseLeaf) <*> (RoseNode x xs) = RoseLeaf
>   (RoseNode x xs) <*> (RoseLeaf) = RoseLeaf
>   (RoseNode y ys) <*> (RoseNode x xs) = RoseNode (y x) (zipWith (<*>) ys xs)


> instance Monad (Rose) where
>   return = pure
>   (RoseLeaf) >>= _ = RoseLeaf
>   (RoseNode x xs) >>= f = RoseNode x' (xs' ++ map (>>= f) xs)
>      where
>         RoseNode x' xs' = f x